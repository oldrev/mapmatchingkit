...
namespace RBush
{
    public partial class RBush<T>
    {
        ...
        #region Search
        private static readonly IReadOnlyList<IImmutableStack<ISpatialData>> EmptyIntersections = new IImmutableStack<ISpatialData>[] { };

        private IReadOnlyList<IImmutableStack<ISpatialData>> DoSearch(in Envelope boundingBox)
        {
            var node = this.root;
        ...
        #region Insert
        private IReadOnlyList<Node> FindCoveringArea(in Envelope area, int depth)
        {
            var path = new List<Node>();
            var node = this.root;
            var area_ = area; //FIX CS1628
            while (true)
            {
                path.Add(node);
                ...
                node = node.Children
                    .Select(c => new { EnlargedArea = c.Envelope.Enlargement(area_).Area, c.Envelope.Area, Node = c as Node, })
        ...
        private int GetBestSplitIndex(IEnumerable<ISpatialData> children)
        {
